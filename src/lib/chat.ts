...
class ChatService {
  ...
  private getAuthHeaders(authToken?: string) {
  ...
  async postAgentMessage(
    ...
    try {
      const response = await fetch(`/api/agent-message/${sessionId}`, {
        method: 'POST',
        headers: this.getAuthHeaders(authToken),
      ...
      const message = error instanceof Error ? error.message : 'Failed to post agent message';
      console.error('Failed to post agent message:', message);
      return { success: false, error: message };
    }
  }
  ...
  async clearMessages(authToken?: string): Promise<ChatResponse> {
    try {
  ...
  async createSession(title?: string, sessionId?: string, firstMessage?: string, authToken?: string): Promise<{ success: boolean; data?: { sessionId: string; title: string }; error?: string }> {
    try {
  ...
  async deleteSession(sessionId: string, authToken?: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async updateSessionTitle(sessionId: string, title: string, authToken?: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async clearAllSessions(authToken?: string): Promise<{ success: boolean; data?: { deletedCount: number }; error?: string }> {
    try {
      const response = await fetch('/api/sessions', {
        method: 'DELETE',
        headers: this.getAuthHeaders(authToken)
  ...
  async updateModel(model: string, authToken?: string): Promise<ChatResponse> {
    try {
